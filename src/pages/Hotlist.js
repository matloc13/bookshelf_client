...
const Hotlist = () => {
    ...
    return (
        <main className="hotlist-container">
            <h2>Hot 50</h2>
            <aside className="loading-aside">{loading && <Loading />}</aside>
            <>
                {hotlist.items &&
                            ...
                            <div key={`${ele.rank}`} className="hotlist-item">
                                ...
                                <div className="add-button-container" key={index + 'button'}>
                                    <span
                                        ...
                                        onClick={(e) => handleClick(e, ele)}
                                    >
                                        {focusCurrent.id === ele.id && gameForm
                                            ? 'close'
                                            : ' add to list'}
                                    </span>
                                    <span
                                        ...
                                        onClick={(e) => handleClick(e, ele)}
                                    >
                                        {focusCurrent.id === ele.id && newList
                                            ? 'close'
                                            : 'new list'}
                                    </span>
                                </div>
                                ...
                                <>
                                    {gameInfo && ele.id === focusCurrent.id && gameInfo && (
                                        <GameInfo set={setGameInfo} gameInfo={gameInfo} />
                                    )}
                                </>
                            </div>
                        );
