    ...
    return (
        <main className="home">
            <nav className="home-nav">
                <button>
                    <Link to="search">Find a game on BGG</Link>
                </button>
                <button>
                    <Link to="hotlist">BGG hot 50</Link>
                </button>
            ...
            {signup === 'show' && !user.isAuthenticated && <Form formType="CREATE" />}

            {infoArt === 'show' && !user.isAuthenticated && (
                <article>
                    Please create an account in order to create your own lists from the bgg
                    database.
                    <button id="infoart" onClick={toggle} className="btn-close">
                        cool
                    </button>
            ...
            <article className="home-user-list">
                {user && user.isAuthenticated ? <UserList /> : <p></p>}
            </article>
        </main>
    );
};
